"""
AMCAF Prototype — Automated Multi-Cloud Compliance Assurance Framework

Rule-based compliance validation engine for AWS, Azure and GCP configurations.
...
# ─────────────────────────────────────────────
# CONTROL LIBRARY
# Maps control IDs to validation functions and
# regulatory traceability metadata.
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# VALIDATION ENGINE
# Provider-specific rule functions that return
# (status, detail) tuples for each configuration.
# ─────────────────────────────────────────────

DORMANCY_THRESHOLD_DAYS = 90  # Accounts inactive beyond this are flagged
AWS_MIN_LOG_RETENTION_DAYS = 365
AZURE_MIN_LOG_RETENTION_DAYS = 90
...
def validate_aws(config: dict[str, Any]) -> list[dict]:
    ...
    # last_used_days = None means credentials have never been used
    dormant = [
        u["username"] for u in iam.get("users", [])
    ...
    # LOG-01: CloudTrail enabled in all configured regions
    ...
    # LOG-02: Log file validation enabled; S3 Object Lock configured;
    #         retention >= 365 days
    log02_failures = []
    ...
    if retention < AWS_MIN_LOG_RETENTION_DAYS:
        log02_failures.append(
            f"retention period {retention} days is below "
            f"{AWS_MIN_LOG_RETENTION_DAYS}-day minimum"
        )
    results.append({
        "control_id": "LOG-02",
        "provider": "AWS",
        ...
        "detail": (
            ...
            if log02_failures
                ...
                "Log file validation enabled; S3 Object Lock configured; "
                "retention >= 365 days"
            )
        ),
        "config_attribute": (
        ...
        for rule in sg.get("inbound_rules", []):
            if rule.get("cidr") == "0.0.0.0/0":
                ...
                if rule.get("port") in management_ports:
                    open_mgmt_sgs.append(
                        f"{sg['group_id']} port {rule['port']}"
                    )
    results.append({
        "control_id": "NET-01",
        "provider": "AWS",
...
def validate_azure(config: dict[str, Any]) -> list[dict]:
    ...
    # IAM-02: MFA enforced via Conditional Access
    ca = config.get("conditional_access", {})
    mfa_policy_active = ca.get("mfa_policy_enabled", False)
    results.append({
        "control_id": "IAM-02",
        "provider": "Azure",
        "status": "PASS" if mfa_policy_active else "FAIL",
        "detail": (
    ...
    # ENC-02: Minimum TLS version on storage accounts
    ...
    activity_log_configured = diag.get("activity_log_enabled", False)
    results.append({
        "control_id": "LOG-01",
        "provider": "Azure",
        "status": "PASS" if activity_log_configured else "FAIL",
        "detail": (
            "Activity Log diagnostic settings configured"
            if activity_log_configured
            else "Azure Activity Log not routed to Log Analytics Workspace"
        ),
        "config_attribute": "diagnostic_settings.activity_log_enabled",
    })

    # LOG-02: Log Analytics Workspace retention >= 90 days;
    #         immutable storage enabled
    log02_failures = []
    retention = diag.get("log_retention_days", 0)
    if retention < AZURE_MIN_LOG_RETENTION_DAYS:
        log02_failures.append(
            f"Log Analytics retention {retention} days is below "
            f"{AZURE_MIN_LOG_RETENTION_DAYS}-day minimum"
        )
...
def validate_gcp(config: dict[str, Any]) -> list[dict]:
    ...
    # Note: GCP public bucket exposure is classified as a network access
    # control (NET-01) rather than an encryption control (ENC-01) because
    # the risk is unauthorised public access rather than data-at-rest
    # protection — public_access_prevention is a network-layer boundary
    # control in GCP's service model.
    ...
    admin_logs_active = audit.get("admin_activity_enabled", False)
    results.append({
        "control_id": "LOG-01",
        "provider": "GCP",
        "status": "PASS" if admin_logs_active else "FAIL",
        "detail": (
            "Admin Activity audit logging is enabled"
            if admin_logs_active
    ...
    # GCP IAM Recommender surfaces unused bindings; modelled here as
    # last_used_days on each service account object.
    iam = config.get("iam", {})
    dormant_sa = [
...
# ─────────────────────────────────────────────
# REPORTING MODULE
# Assembles validation results into structured
# compliance output with regulatory traceability.
# ─────────────────────────────────────────────
...
def run_scenario(scenario_id: str, aws_cfg: dict, azure_cfg: dict, gcp_cfg: dict):
    print(f"\n{'='*60}")
    ...
    for f in report["findings"]:
        ...
        if f["status"] == "FAIL":
            refs = ", ".join(f["regulatory_refs"])
            print(f"       Regulatory refs: {refs}")
...
# SC-01: Fully compliant baseline — all controls pass
sc01_aws = {
    "iam": {
...
sc01_gcp = {
    ...
    "iam": {
        "service_accounts": [
            {"name": "svc-dataflow@proj.iam", "last_used_days": 14},
        ]
    },
    "vpc": {"firewall_rules": []},
}
...
# SC-03: Encryption disabled (AWS) + public bucket (GCP) + weak TLS (Azure)
sc03_aws = {
    "iam": {
        "policies": [{"name": "ReadOnlyPolicy", "actions": ["s3:GetObject"]}],
...
sc03_gcp = {
    ...
    "iam": {
        "service_accounts": [
            {"name": "svc-dataflow@proj.iam", "last_used_days": 20},
        ]
    },
    "vpc": {"firewall_rules": []},
}
...
# SC-05: Open SSH security group (AWS) + permissive firewall (GCP)
sc05_aws = {
    "iam": {
        "policies": [{"name": "ReadOnlyPolicy", "actions": ["s3:GetObject"]}],
...
sc06_aws = {
    "iam": {
        ...
        "users": [
            ...
            {"username": "svc-legacy",   "mfa_enabled": True, "last_used_days": 142},  # dormant
            {"username": "audit-reader", "mfa_enabled": True, "last_used_days": None}, # never used
        ],
    },
    "s3": {"buckets": [{"name": "prod-data", "default_encryption_enabled": True}]},
    "cloudtrail": {
        "trails": [{"name": "mgmt-trail", "is_logging": True}],
        "log_file_validation_enabled": True,
        "s3_object_lock_enabled": False,  # FAIL — no WORM protection
        "retention_days": 180,            # FAIL — below 365-day minimum
    },
    "ec2": {"security_groups": []},
}
sc06_azure = {
    ...
    "diagnostic_settings": {
        "activity_log_enabled": True,
        "log_retention_days": 60,         # FAIL — below 90-day minimum
        "immutable_storage_enabled": True,
    },
}
sc06_gcp = {
    ...
    "iam": {
        "service_accounts": [
            ...
            {"name": "svc-archive@proj.iam",   "last_used_days": 210},  # dormant
        ]
    },
    "vpc": {"firewall_rules": []},
}
...
if __name__ == "__main__":
    run_scenario(
        ...
        sc01_aws, sc01_azure, sc01_gcp,
    )
    run_scenario(
        "SC-02: Over-privileged IAM (AWS) / MFA Disabled (Azure)",
        sc02_aws, sc02_azure, sc02_gcp,
    )
    run_scenario(
        "SC-03: Encryption Disabled (AWS) / Weak TLS (Azure) / Public Bucket (GCP)",
        sc03_aws, sc03_azure, sc03_gcp,
    )
    run_scenario(
        "SC-04: Logging Disabled (All Providers)",
        sc04_aws, sc04_azure, sc04_gcp,
    )
    run_scenario(
        "SC-05: Open SSH (AWS) / Permissive Firewall (GCP)",
        sc05_aws, sc05_azure, sc05_gcp,
    )
    run_scenario(
        "SC-06: Dormant Accounts (AWS, GCP) / Insufficient Log Retention (AWS, Azure)",
        sc06_aws, sc06_azure, sc06_gcp,
    )
